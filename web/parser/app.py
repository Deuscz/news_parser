...
def articles_list():
    articles = Article.query.join(Source).filter(Article.published_date == datetime.date.today())
    try:
    ...
    return render_template('article_list.html', articles=articles, sport_articles=sport_articles)
...
def statistics():
    ...
    for source in Source.query.filter(Source.category.in_(('health', 'politics'))):
        health_articles = list(Article.query.filter(Article.url_id == source.id, Article.category == 'health'))
        politics_articles = list(Article.query.filter(Article.url_id == source.id, Article.category == 'politics'))
        last_news_date = Article.query.filter(Article.url_id == source.id).order_by(
            desc(Article.published_date)).first()
    ...
    try:
        files = [f.split('_')[1][:-4] for f in listdir('sport_files') if isfile(join('sport_files', f))]
        ...
        file_statistics = {}
    return render_template('statistics.html', db_statistics=db_statistics, file_statistics=file_statistics)
