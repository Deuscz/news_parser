...
date_formats = ['%a, %d %b %Y %H:%M:%S', '%a, %d %B %Y %H:%M:%S', '%a %d %b %Y %H:%M:%S', '%d %b %Y %H:%M:%S']
...
def sport_to_json(ch, method, properties, body):
    ...
    filename = "sport_" + date_today.strftime("%Y_%m_%d") + ".txt"
    articles = json.loads(body)
    data = []
    for article in articles:
        for date_format in date_formats:
            try:
                published_date = datetime.datetime.strptime(' '.join(article[3].split(' ')[:-1]), date_format).date()
                break
            except ValueError:
                continue
...
def consumer():
    ...
    channel.basic_consume(queue='sport', on_message_callback=sport_to_json, auto_ack=True)
    channel.basic_consume(queue='health', on_message_callback=save_articles_to_db, auto_ack=True)
    channel.basic_consume(queue='politics', on_message_callback=save_articles_to_db, auto_ack=True)
